...
const {
  ...
  distanceTo,
  escapeObstacle,
} = require("./UtilitiesFunctions.jsx");
class Global {
  constructor() {
  ...
  newPredator(x_, y_, energy_, gene_string_) {
    ...
    let temp_predator = new Predator(
      1,
      this.m_predator_generation[0],
      x_,
      y_,
  ...
  initializeGrazerInfo(
    g_energy_input_,
    g_energy_output_,
    g_energy_to_reproduce_,
    g_maintain_speed_,
    g_max_speed_
  ) {
    this.m_grazer_stuff = new GrazerInfo(
  ...
  intializePredatorInfo(
    p_maintain_speed_,
    p_energy_output_,
    p_energy_to_reproduce_,
    p_max_offspring_,
    p_gestation_,
    p_offspring_energy_
  ) {
  ...
  grazerDecisionTree(grazer_) {
    ...
    for (let obstruction of obstructions) {
      let obs_distance = distanceTo(
        [grazer_.m_x_pos, grazer_.m_y_pos],
        [obstruction.m_x_pos, obstruction.m_y_pos]
      );
      if (obs_distance < obstruction.m_radius) {
        escapeObstacle(
          grazer_,
          obstruction,
          [this.m_world_size_x, this.m_world_size_y],
          obstructions
        );
      }
    }
    // find predator in sight.
    let target_x_y = findPredator(
      grazer_.m_x_pos,
      grazer_.m_y_pos,
      grazer_pred_sight,
      this.m_pred_list,
    ...
    if (grazer_.m_energy < 1) {
      ...
      grazer_.moveFlee(
        target_x_y,
        this.m_grazer_stuff.m_max_speed,
        this.m_grazer_stuff.m_energy_out,
        obstructions,
        [this.m_world_size_x, this.m_world_size_y],
        this.m_grazer_stuff.maintain_speed
      );
    } else {
      ...
      if (grazer_.m_energy > this.m_grazer_stuff.m_energy_to_reproduce) {
        ...
        if (target) {
          let distance_to_food = distanceTo(
            [grazer_.m_x_pos, grazer_.m_y_pos],
            [target.m_x_pos, target.m_y_pos]
          );
          let speed = this.m_grazer_stuff.m_max_speed;

          if (
            distance_to_food >
            target.m_radius + this.m_grazer_stuff.m_max_speed / 60 + 5
          ) {
            grazer_.moveSeek(
              target,
              speed,
            ...
            if (
              distance_to_food <
                target.m_radius + this.m_grazer_stuff.m_max_speed / 60 &&
              distance_to_food > 1 + target.m_radius
            ) {
  ...
  predatorDecisionTree(pred_) {
    ...
    if (pred_.m_energy < 1 || pred_.nan()) {
      ...
      if (target != null) {
        ...
        pred_.moveSeek(
          target,
          this.m_predator_stuff.m_maintain_speed,
          this.m_predator_stuff.m_energy_out,
          [this.m_world_size_x, this.m_world_size_y],
          obstructions
        );
        if (
  ...
  plantDecisionTree(plant_) {
    ...
    if (
      plant_.m_radius * 2 <= this.m_plant_stuff.m_max_size &&
      plant_.m_lifetime >= 10
    ) {
      plant_.m_radius =
        plant_.m_radius +
        this.m_plant_stuff.m_max_size *
          (this.m_plant_stuff.m_growth_rate / 100); // grow by 1% max
    }
  ...
  printEnts() {
    let return_string =
      ...
      this.m_world_size_y +
      "," +
      this.m_plant_list.length +
      "," +
      this.m_grazer_list.length +
      "," +
      this.m_pred_list.length +
      "," +
      this.m_obs_list.length +
      ",";
