...
class Predator extends Entity {
  ...
  eat(entity_) {
    this.m_energy += entity_.m_energy * 0.9; //gain a fixed amount of energy
  ...
  reproduce(predators_array_, target_, generation_array_) {
    ...
    generation_array_[next_generation - 1] =
      generation_array_[next_generation - 1] + 1;
    let next_entity = generation_array_[next_generation - 1];

    var g_string = "";
    var temp = "";
    temp = temp.concat(
      this.m_genes_obj.m_aggro[Math.floor(Math.random * 2)],
      target_.m_genes_obj.m_aggro[Math.floor(Math.random * 2)]
    );
    if (temp === "aA") {
      temp = "Aa";
    }
    ...
    temp = "";
    temp = temp.concat(
      this.m_genes_obj.m_speed[Math.floor(Math.random * 2)],
      target_.m_genes_obj.m_speed[Math.floor(Math.random * 2)]
    );
    if (temp === "sS") {
      temp = "Ss";
    }
    ...
    temp = temp.concat(
      this.m_genes_obj.m_strength[Math.floor(Math.random * 2)],
      target_.m_genes_obj.m_strength[Math.floor(Math.random * 2)]
    );
    if (temp === "fF") {
      temp = "Ff";
    }
    ...
    const offspring = new Predator(
      next_generation,
      next_entity,
      this.m_x_pos + Math.random(),
      this.m_y_pos + Math.random(),
      this.m_radius,
      0,
      Math.floor(this.energy / 2),
      gene_copy
    );
  ...
  moveTo(entity) {
    const angle = Math.atan2(
      entity.m_y_pos - this.m_y_pos,
      entity.m_x_pos - this.m_x_pos
    );
  ...
  moveSeek(target_, speed_time_, energy_use_, world_size_, obstructions_) {
    ...
    let steering = seek(
      [this.m_x_pos, this.m_y_pos],
      [target_.m_x_pos, target_.m_y_pos],
      this.m_speed
    );
    changePosition(
      this,
