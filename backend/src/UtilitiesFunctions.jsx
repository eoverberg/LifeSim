...
function changePosition(
  ...
  const radian_array = [
    0, 0.26, -0.26, 0.52, -0.52, 0.78, -0.78, 1.04, -1.04, 1.56, -1.56, 2.08,
    -2.08, 2.6, -2.6, 3.12,
  ];
  ...
  for (let radian of radian_array) {
    ...
    if (
      new_position[0] > world_size_[0] ||
      new_position[1] > world_size_[1] ||
      new_position[0] < 0 ||
      new_position[1] < 0
    ) {
      needsChange = true;
    ...
    if (obstructions_) {
      for (let obstruction of obstructions_) {
        let distance_to_target = distanceTo(new_position, [
          obstruction.m_x_pos,
          obstruction.m_y_pos,
        ]);
...
function escapeObstacle(entity_, target_, world_size_, obstructions_) {
  ...
  for (let radian of radian_array) {
    ...
    // get coordinates from orientation
    steering[0] =
      (target_.m_radius + entity_.m_radius) * Math.cos(temp_orientaton);
    steering[1] =
      (target_.m_radius + entity_.m_radius) * Math.sin(temp_orientaton);
    // console.log("sc steering: " + steering[0] +","+ steering[1])
    // still looking at obstruction pov
    new_position = [
      target_.m_x_pos + steering[0],
      target_.m_y_pos + steering[1],
    ];
    // console.log("position:" + new_position[0] +","+ new_position[1])
    // if target is outside of world try the next one
    if (
...
function checkPath(source_, radius__, target__, obstacles__) {
  ...
  for (let obstacle of obstacles_) {
    ...
    if (
      distanceTo(target_, [obstacle.m_x_pos, obstacle.m_y_pos]) <
      radius_ ** 2 + obstacle.m_radius ** 2
    ) {
      let line_x;
      let line_y;
      let position =
        ((obstacle.m_x_pos - s_x) * x_diff +
          (obstacle.m_y_pos - s_y) * y_diff) /
        distance;
      ...
      if (dis_to_line <= obstacle.m_radius) {
        ...
        let obs_edge = [
          obstacle.m_x_pos + edgeDist[0],
          obstacle.m_y_pos + edgeDist[1],
        ];
...
function checkLOS(s_x_, s_y_, x_diff_, y_Diff_, distance_, obstacles_) {
  ...
  for (let obstacle of obstacles_) {
    ...
    let position =
      ((obstacle.m_x_pos - s_x_) * x_diff_ +
        (obstacle.m_y_pos - s_y_) * y_Diff_) /
      distance_;
    // if 0 or 1 obstacle is closest to target or source and not blocking los
    if (position < 1 && position > 0) {
      // find closest (x,y) on line to obstacle
      ...
      // find distance from obstacle to line
      let dis_to_line =
        (obstacle.m_x_pos - line_x) ** 2 + (obstacle.m_y_pos - line_y) ** 2;
...
function findClosest(
  ...
  for (let ent of entities_) {
    ...
    if (distance_2 < dis_check && distance_2 !== 0) {
      ...
      if (
        distance_2 < smell_distance_ ||
        !checkLOS(s_x_, s_y_, x_diff, y_diff, distance_2, obstructions_)
      ) {
